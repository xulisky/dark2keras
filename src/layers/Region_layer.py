...
#    return (input_shape[0], rows, cols, int(len(self.anchors)/2), int(5+self.n_class))
  ...
  def _transform(self, inputs):
    ...
    if self.softmax: # softmax or not <<<
      b_c_probs = K.softmax(net_outs[..., 5:])
    else:
      b_c_probs = net_outs[..., 5:]
    ...
    b_xy = (K.sigmoid(t_xy) + meshgrid) / grid_hw_tensor # 0~1.0
    b_wh = (K.exp(t_wh) * anchors_tensor) / grid_hw_tensor # 0~1.0
...
#    return preds # [?,13,13,5,85] 85 correspond to absolute coords(4), conf(1), class(80)
    preds = K.reshape(preds, [K.shape(preds)[0],-1,int(5+self.n_class)])
  ...
  def get_config(self):
    ...
    base_config = super(Region, self).get_config()
